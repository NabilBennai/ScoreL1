...
import { applyDixonColes, dixonColesTau } from "../../src/lib/model/dixon-coles"
import {
...
describe("applyDixonColes", () => {
  it("returns the Poisson grid when rho is 0", () => {
    const poisson = normalizeScoreGrid(buildPoissonScoreGrid(1.5, 1.2, 12))

    const corrected = applyDixonColes(poisson, 1.5, 1.2, 0)
  ...
  it("returns a normalized grid", () => {
    ...
    const corrected = applyDixonColes(poisson, 1.5, 1.2, -0.1)
  ...
  it("rejects invalid corrections", () => {
    ...
    expect(() => applyDixonColes(poisson, 1.5, 1.2, 10)).toThrow()
  })
})
