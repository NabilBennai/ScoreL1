...
function validateMarkets(markets: FairMarketInput): void {
  ...
  const oneXTwoTotal =
    markets.oneXTwo.home + markets.oneXTwo.draw + markets.oneXTwo.away
...
function computeLoss(
  ...
  const grid = applyDixonColes(poisson, lambdaHome, lambdaAway, rho)
...
export function fitGoalModel(markets: FairMarketInput, rho = 0): GoalModelFit {
  ...
  for (let lambdaHome = 0.1; lambdaHome <= 5; lambdaHome += 0.1) {
    for (let lambdaAway = 0.1; lambdaAway <= 5; lambdaAway += 0.1) {
      const loss = computeLoss(lambdaHome, lambdaAway, rho, markets)
